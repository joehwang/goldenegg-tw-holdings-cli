...
class Settings(BaseSettings):
    ...
    def get_esun_settings(self) -> EsunSettings:
        ...
        if self.egg_debug:
            env_file = "broker/esun/.env.test"      
        else:
            env_file = "broker/esun/.env"  
    ...
    def get_fubon_settings(self) -> FubonSettings:
        ...
        if self.egg_debug:
            env_file = "broker/fubon/.env.test"      
        else:
            env_file = "broker/fubon/.env"  
        return FubonSettings(_env_file=env_file)

    def get_masterlink_settings(self) -> MasterlinkSettings:
        """動態載入元富證券設定"""
        if self.egg_debug:
            env_file = "broker/masterlink/.env.test"      
        else:
            env_file = "broker/masterlink/.env"  
        return MasterlinkSettings(_env_file=env_file)

    def get_sinopac_settings(self) -> SinopacSettings:
        """動態載入永豐金證券設定"""
        if self.egg_debug:
            env_file = "broker/sinopac/.env.test"      
        else:
            env_file = "broker/sinopac/.env"
